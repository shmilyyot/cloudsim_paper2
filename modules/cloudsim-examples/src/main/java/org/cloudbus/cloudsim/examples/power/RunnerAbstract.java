...
public abstract class RunnerAbstract {
	...
	public RunnerAbstract(
		...
		start(
				...
				outputFolder,
				getVmAllocationPolicy(vmAllocationPolicy, vmSelectionPolicy, parameter, placement));
	}
	...
	protected void initLogOutput(
			...
			String vmSelectionPolicy,
			String parameter,
			String placement) throws IOException, FileNotFoundException {
		...
		if (isEnableOutput() && outputToFile) {
			...
			File file = new File(outputFolder + "/log/"
					+ getExperimentName(workload, vmAllocationPolicy, vmSelectionPolicy, parameter, placement) + ".txt");
			file.createNewFile();
	...
	protected VmAllocationPolicy getVmAllocationPolicy(
		...
		if (vmAllocationPolicyName.equals("iqr")) {
		...
		} else if(vmAllocationPolicyName.equals("trend")){
			PowerVmAllocationPolicyMigrationAbstract fallbackVmSelectionPolicy = new PowerVmAllocationPolicyMigrationStaticThreshold(
					hostList,
					vmSelectionPolicy,
					0.8);
			vmAllocationPolicy = new PowerVmAllocationPolicyPredictTrend(
					...
					parameter,
					fallbackVmSelectionPolicy);
		}else if(vmAllocationPolicyName.equals("trendthr")) {
			vmAllocationPolicy = new PowerVmAllocationPolicyPredictTrendTHR(
					hostList,
					vmSelectionPolicy,
					parameter);
		}
		else if(vmAllocationPolicyName.equals("peap")){
			vmAllocationPolicy = new PowerVmAllocationPolicyMigrationPEAP(
					hostList,
					vmSelectionPolicy,
					parameter);
		}else if(vmAllocationPolicyName.equals("capmop")){
			vmAllocationPolicy = new PowerVmAllocationPolicyMigrationCAPMOP(
					hostList,
					vmSelectionPolicy,
					parameter);
		}else if(vmAllocationPolicyName.equals("upvmc")){
			vmAllocationPolicy = new PowerVmAllocationPolicyMigrationUPVMC(
					hostList,
					vmSelectionPolicy,
					parameter);
		...
		vmAllocationPolicy.setPlacement(placement);
	...
	protected PowerVmSelectionPolicy getVmSelectionPolicy(String vmSelectionPolicyName) {
		...
		}else if(vmSelectionPolicyName.equals("trend")){
			vmSelectionPolicy = new PowerVmSelectionPolicyPredictTrend();
		}else if(vmSelectionPolicyName.equals("peacr")){
