...
public class PowerVm extends Vm {
	public String getVmTrendType() {
		return vmTrendType;
	}

	public void setVmTrendType(String vmTrendType) {
		this.vmTrendType = vmTrendType;
	}

	public String vmTrendType = null;
	...
	public PowerVm(
		...
		super(id, userId, mips, pesNumber, ram, bw, size, vmm, cloudletScheduler, workload);
...
//			}
	...
	public double getLastTrend(){
		double currentUtilization = this.getTotalUtilizationOfCpu(CloudSim.clock());
		return currentUtilization - getUtilizationHistory().get(0);
	}

	public double[] getHistoryTrend(){
		List<Double> historyUtilization = this.getUtilizationHistory();
		double[] historyUtilizationArr = MathUtil.doubleListToArray(historyUtilization);
		int len = historyUtilizationArr.length;
		double[] historyTrend = new double[len];
		for(int i = 0; i < len - 1; ++i){
			historyTrend[i] = (historyUtilizationArr[i+1] - historyUtilizationArr[i]) * this.getMips();
		}
		historyTrend[len-1] = (this.getTotalUtilizationOfCpuMips(CloudSim.clock()) - historyUtilizationArr[len-1]) * this.getMips();
		return historyTrend;
	}
