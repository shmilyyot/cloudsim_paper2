...
public class PowerHost extends HostDynamicWorkload {
	...
	private double ppr = 1.0;
	private double peakPeff = 0.0;
	/**
	...
	long getCpuOffer(){
		return Math.round((double)getTotalMips() / 1000 * Math.max(getPpr() - getUtilizationOfCpu(), 0));
	}

	public double getPeakPeff() {
		return peakPeff;
	}

	public void setPeakPeff(double peakPeff) {
		this.peakPeff = peakPeff;
	}
}
