...
public class Vm {
	...
	public double[] getPredictUtilization() {
		return predictUtilization;
	}

	public void setPredictUtilization(double[] predictUtilization) {
		this.predictUtilization = predictUtilization;
	}

	private double[] predictUtilization;

	// 每次给出当前时刻的下一个时刻的预测值，当前300秒，给出600秒的预测值
	public double getPredictUtilization(double time) {
		int index = ((int) time / 300) + 1;
		if(index < predictUtilization.length)
		{
			return predictUtilization[index];
		}
		return predictUtilization[predictUtilization.length - 1];
	}
	...
	public Vm(
		...
		try {
			ObjectInputStream input = new ObjectInputStream(Files.newInputStream(Paths.get("D:\\java_workspace\\cloudsim-cloudsim-4.0\\modules\\cloudsim-examples\\target\\classes\\workload\\planetlab\\20110303\\predict\\vm_"+id+".obj")));
